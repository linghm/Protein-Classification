...
def conv2d(input, output, name, is_training, kernel_size, bn_decay=None, 
            reuse=None, with_bn=False, activation=tf.nn.relu):
    if with_bn:
        input = tf.layers.batch_normalization(input, momentum=0.98, training=is_training, name=name+'bn')
...
def shellconv(pts, fts_prev, qrs, is_training, tag, K, D, P, C, with_local, bn_decay=None):
    indices = pf.knn_indices_general(qrs, pts, K, True)   #(N,P,K,2)
    #这里的N是batchsize
    ...
    if with_local:
        ...
        nn_fts_local = dense(nn_fts_local, C_pts_fts, is_training, tag + 'nn_fts_from_pts',bn_decay=bn_decay)    #(N,P,K,64)
    else:
        nn_fts_local = nn_pts_local

    if fts_prev is not None:
        fts_prev = tf.gather_nd(fts_prev, indices, name=tag + 'fts_prev')  # (N, P, K, num_feature)   #这里不一定是3吧
        #pts_X_0 = tf.concat([nn_fts_local,fts_prev], axis=-1)
        #modify here, we just use the node features
        pts_X_0 = fts_prev
    else:
    ...
    fts_X = conv2d(feat_max, C, name=tag+'conv', is_training=is_training, kernel_size=[1,feat_max.shape[-2]])    #(N,P,1,C)
    fts_X = tf.squeeze(fts_X, axis=-2)  #(N,P,C)
    return fts_X
...
if __name__ == '__main__':
    with tf.Graph().as_default():
        with tf.device('/gpu:0'):
            ...
            outputs = get_model(inputs, tf.constant(True), sconv_params, None, fc_params)
            print(outputs)
