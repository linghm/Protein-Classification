...
flags.DEFINE_boolean('balanced_data', False, 'whether we use balanced data')
...
if FLAGS.dataset == 'WT':
    ...
    if FLAGS.dataset_type == 'binary':
        if FLAGS.balanced_data:
            print('balanced data')
            features = features[:1932+1932]
            edge_features = edge_features[:1932+1932]
            labels = labels[:1932+1932]
            splits = [[0,2800],[2800,3300],[3300,3800]]
        else:
...
#node features with or without position feature
...
print('head of index:{}'.format(index[:10]))
...
def evaluate(sess, network, features_test, edge_features_test, labels_test,  placeholders):
    t_test = time.time()
...
vacc_mx = 0.0
curr_step = 0
checkpt_path = 'pre_trained/{}-{}-epochs{}-batchsize{}-lr{}-hidden2-{}-dropout{}-weight_decay{}-hidden3-{}-mu{}-run{}-seed{}'.format(FLAGS.dataset,FLAGS.dataset_type,FLAGS.epochs,FLAGS.batchsize,
...
for epoch in range(FLAGS.epochs):
    ...
    # validation
    val_loss, val_acc, val_duration = evaluate(sess, network, features_val, edge_features_val, labels_val, GCN_placeholders)
...
test_loss, test_acc, test_duration = evaluate(sess, network, features_test, edge_features_test, labels_test,GCN_placeholders)
print("Epoch:", '%04d' % (epoch_early_model+1), "test_loss=", "{:.5f}".format(test_loss), "test_acc=", "{:.5f}".format(test_acc))

print(time.time()-t)
filename = '_result_earlystop_kipf.txt'
with open(filename,'a') as f: 
    f.write("##{}-{}: with_position:{},epochs:{},batchsize:{},lr:{},hidden2:{},dropout:{},weight_decay:{},hidden3:{},mu:{},balanced data:{},seed:{}\n".format(FLAGS.dataset,FLAGS.dataset_type,FLAGS.with_position,FLAGS.epochs,FLAGS.batchsize,
            FLAGS.learning_rate, FLAGS.hidden2,FLAGS.dropout,FLAGS.weight_decay,FLAGS.hidden3,FLAGS.mu,FLAGS.balanced_data,FLAGS.seed))
