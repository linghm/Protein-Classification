...
import utils
from neural_networks_shellnet import GCNGraphs
...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'Triple', 'which dataset to load')  # PROTEINS
...
flags.DEFINE_integer('hidden3', 16, 'Number of units in hidden layer 2.')

flags.DEFINE_integer('P', 16, 'Number of points in queries.')
...
flags.DEFINE_boolean('with_local', False, 'whether use the with local in shellconv operation')
...
#features = features_total[:,:,[i for i in range(27)]+[30]]
...
GCN_placeholders = {
    ...
    # modify here
    'points': tf.placeholder(tf.float32, shape = (None, coords.shape[1],coords.shape[2])),
...
def evaluate(sess, network, features_test, edge_features_test, labels_test,  coords_test, placeholders):
    t_test = time.time()
    ...
    for i in range(0, total, FLAGS.batchsize):
        ...
        test_dict = utils.build_dictionary_GCN_shellnet(features_batch, edge_features_batch, labels_batch, coords_batch, GCN_placeholders)
        #modify here
        test_dict.update({GCN_placeholders['is_training']: False})
        # Test step 
...
vacc_mx = 0.0
curr_step = 0
checkpt_path = 'pre_trained_shellnet/{}-{}-epochs{}-batchsize{}-lr{}-hidden2-{}-dropout{}-weight_decay{}-hidden3-{}-mu{}-run{}-seed{}'.format(FLAGS.dataset,FLAGS.dataset_type,FLAGS.epochs,FLAGS.batchsize,
...
for epoch in range(FLAGS.epochs):
    ...
    for i in range(0, total, FLAGS.batchsize):
        ...
        train_dict = utils.build_dictionary_GCN_shellnet(features_batch, edge_features_batch, labels_batch,  coords_batch, GCN_placeholders)
        train_dict.update({GCN_placeholders['dropout']: FLAGS.dropout})
        #modify here
        train_dict.update({GCN_placeholders['is_training']: True})
        # Training step 
    ...
    # validation
    val_loss, val_acc, val_duration = evaluate(sess, network, features_val, edge_features_val, labels_val, coords_val, GCN_placeholders)
...
test_loss, test_acc, test_duration = evaluate(sess, network, features_test, edge_features_test, labels_test, coords_test, GCN_placeholders)
print("Epoch:", '%04d' % (epoch_early_model+1), "test_loss=", "{:.5f}".format(test_loss), "test_acc=", "{:.5f}".format(test_acc))
  

print(time.time()-t)
filename = '_result_shellnet_earlystop.txt'
with open(filename,'a') as f: 
    f.write("##{}-{} shellnet: with_local:{},with_position:{},epochs:{},batchsize:{},lr:{},hidden2:{},dropout:{},weight_decay:{},hidden3:{},mu:{},seed{}\n".format(FLAGS.dataset,FLAGS.dataset_type,FLAGS.with_local, FLAGS.with_position,FLAGS.epochs,FLAGS.batchsize,
            FLAGS.learning_rate, FLAGS.hidden2,FLAGS.dropout,FLAGS.weight_decay,FLAGS.hidden3,FLAGS.mu,FLAGS.seed))
