...
class ConvolutionalLayer(Layer):
    def __init__(self, input_dim, output_dim, placeholders, dropout,
        ...
        self.bias = bias
        self.num_edge_feature = self.edge_features.get_shape().as_list()[3]
    ...
    def _call(self, inputs):
        ...
        num_nodes = self.edge_features.get_shape().as_list()[1]
       
        edge_features = tf.reshape(self.edge_features,[FLAGS.batchsize * num_nodes *num_nodes, self.num_edge_feature])
        ...
        dist_features = batch_distance_matrix(self.coords) 
        dist_features = tf.reshape(dist_features, [FLAGS.batchsize, num_nodes, num_nodes])
        
        edge_features = [tf.nn.softmax(edge_features+self.bias_mat[i]) for i in range(len(self.bias_mat))]
        #注意这里要有一个负号，才能使得距离越小权重越大
        dist_features = [tf.nn.softmax(-dist_features+self.bias_mat[i]) for i in range(len(self.bias_mat))]
        ...
        edge_features = tf.nn.softmax(edge_features/FLAGS.tau)
        '''
        adj_edge = [block_diagonal(edge_features[i]) for i in range(len(edge_features))]
        adj_dist = [block_diagonal(dist_features[i]) for i in range(len(dist_features))]
        support= adj_edge + adj_dist
        #use a renormalization trick
        support = [preprocess_adj(adj, True, False) for adj in support]
